...
console.log(iter6.next());
*/
//Функция-генератор для пагинации данных
const data1 = {
...
function* paginate(data, pageSize) {
  ...
  while (currentIndex < data.length) {
    newData = data.slice(currentIndex, currentIndex + pageSize);
    currentIndex = currentIndex + pageSize;
    yield newData;
  }
}
/*
for (const page of paginate(data, pageSize)) {
  console.log(JSON.stringify(page));
}
  */
//Реализация итератора для обхода дерева в глубину
const data2 = {
  value: "A",
  children: [
    {
...
*/
//Объединение и сортировка потоков
...
function* mergeSortedStreamsGenerator(streams) {
  for (const steam of streams) {
    steam.sort;
    yield* mergeSortedStreamsGenerator(streams);
  }
  console.log(streams);
}
...
function* mergeSortedStreamsGenerator(streams) {
  // 1. Инициализируем состояние каждого потока
  const heads = streams.map((stream) => {
  ...
  // 2. Цикл пока есть хотя бы один незавершенный поток
  while (heads.some((head) => !head.done)) {
    let minHead = null;

    // 3. Ищем активный поток с минимальным значением
    for (const head of heads) {
      if (head.done) continue;
      if (minHead === null || head.value < minHead.value) {
        minHead = head;
      }
    }

    // 4. Возвращаем минимальный элемент
    yield minHead.value;

    // 5. Сдвигаем только тот итератор, чей элемент мы забрали
    const next = minHead.stream.next();
    minHead.value = next.value;
    minHead.done = next.done;
  }
}
